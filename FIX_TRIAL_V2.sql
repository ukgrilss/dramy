-- =================================================================
-- FIX: INITIALIZE TRIAL BALANCE FOR FREE USERS (V2 - NEW NAME)
-- =================================================================

-- 1. Ensure trial_balance column exists (Default 10 minutes = 600 seconds)
ALTER TABLE profiles 
ADD COLUMN IF NOT EXISTS trial_balance INTEGER DEFAULT 600;

-- 2. Create V2 function to bypass "not unique" errors
-- We use a NEW NAME to avoid conflicts with the old messed up functions
CREATE OR REPLACE FUNCTION register_trial_access_v2(
  p_ip_address TEXT,
  p_fingerprint TEXT,
  p_user_agent TEXT,
  p_user_id UUID
...
BEGIN
  ...
  -- Crucial: We set trial_balance here!
...
GRANT EXECUTE ON FUNCTION register_trial_access_v2 TO anon, authenticated, service_role;

SELECT 'CORREÇÃO V2 APLICADA COM SUCESSO' as status;
